...
# На Windows этот раздел не выполняется: клиент прошивки для хоста собирается
# gcc против OpenSSL и mbedTLS. Раздел оставлен ради одинаковой нумерации с
# check-all.sh — сборка проверяется на Linux и в CI.
Warn "Linux-сборка прошивки на Windows пропущена — проверяется на Linux и в CI"
